...
By default, a DM from an unknown sender triggers **pairing**: the bot replies with a 6-character code and drops the message. You run `/telegram-durable:access pair <code>` from your assistant session to approve them. Once approved, their messages pass through.

All state lives in `~/.claude/channels/telegram/access.json`. The `/telegram-durable:access` skill commands edit this file; the server re-reads it on every inbound message, so changes take effect without a restart. Set `TELEGRAM_ACCESS_MODE=static` to pin config to what was on disk at boot (pairing is unavailable in static mode since it requires runtime writes).
...
| Policy | Behavior |
| --- | --- |
| `pairing` (default) | Reply with a pairing code, drop the message. Approve with `/telegram-durable:access pair <code>`. |
...
```
/telegram-durable:access policy allowlist
```
...
```
/telegram-durable:access allow 412587349
/telegram-durable:access remove 412587349
```
...
```
/telegram-durable:access group add -1001654782309 --no-mention
/telegram-durable:access group add -1001654782309 --allow 412587349,628194073
/telegram-durable:access group rm -1001654782309
```
...
```
/telegram-durable:access set mentionPatterns '["^hey claude\\b", "\\bassistant\\b"]'
```

## Delivery

Configure outbound behavior with `/telegram-durable:access set <key> <value>`.
...
```
/telegram-durable:access set ackReaction 👀
/telegram-durable:access set ackReaction ""
```
...
| Command | Effect |
| --- | --- |
| `/telegram-durable:access` | Print current state: policy, allowlist, pending pairings, enabled groups. |
| `/telegram-durable:access pair a4f91c` | Approve pairing code `a4f91c`. Adds the sender to `allowFrom` and sends a confirmation on Telegram. |
| `/telegram-durable:access deny a4f91c` | Discard a pending code. The sender is not notified. |
| `/telegram-durable:access allow 412587349` | Add a user ID directly. |
| `/telegram-durable:access remove 412587349` | Remove from the allowlist. |
| `/telegram-durable:access policy allowlist` | Set `dmPolicy`. Values: `pairing`, `allowlist`, `disabled`. |
| `/telegram-durable:access group add -1001654782309` | Enable a group. Flags: `--no-mention` (also requires disabling privacy mode), `--allow id1,id2`. |
| `/telegram-durable:access group rm -1001654782309` | Disable a group. |
| `/telegram-durable:access set ackReaction 👀` | Set a config key: `ackReaction`, `replyToMode`, `textChunkLimit`, `chunkMode`, `mentionPatterns`. |
